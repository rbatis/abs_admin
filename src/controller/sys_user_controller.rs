...
pub async fn login(arg: web::types::Json<SignInDTO>) -> impl Responder {
...
pub async fn add(arg: web::types::Json<UserAddDTO>) -> impl Responder {
    let vo = CONTEXT.sys_user_service.add(arg.0).await;
    return RespVO::from_result(&vo).resp_json();
}

pub async fn page(arg: web::types::Json<UserRolePageDTO>) -> impl Responder {
    let vo = CONTEXT.sys_user_role_service.page(&arg.0).await;
    return RespVO::from_result(&vo).resp_json();
}

pub async fn detail(arg: web::types::Json<IdDTO>) -> impl Responder {
    let vo = CONTEXT.sys_user_service.detail(&arg.0).await;
    return RespVO::from_result(&vo).resp_json();
}

pub async fn update(arg: web::types::Json<UserEditDTO>) -> impl Responder {
