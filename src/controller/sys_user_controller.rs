...
// pub async fn login(arg: Json<SignInDTO>) -> impl Responder {
...
pub async fn info(req: Request) -> impl IntoResponse {
    ...
    return match token {
        Some(token) => {
            ...
            if token.is_err() {
                return RespVO::<SignInVO>::from_error(token.err().unwrap().to_string()).json();
            }
        ...
        _ => RespVO::<SignInVO>::from_error(error_info!("access_token_empty")).json(),
    };
}
