...
pub async fn info(req: HttpRequest) -> impl Responder {
    ...
    match token {
        Some(token) => {
            ...
            if token.is_err() {
                return HttpResponse::Ok().json(RespVO::<SignInVO>::from_error(token.err().unwrap().to_string()));
            }
            let user_data = CONTEXT
                .sys_user_service
                .get_user_info_by_token(&token.unwrap())
                .await;
            HttpResponse::Ok().json(RespVO::from_result(user_data))
        }
        _ => HttpResponse::Ok().json(RespVO::<SignInVO>::from_error(error_info!("access_token_empty"))),
    }
}
...
pub async fn update(arg: web::Json<UserEditDTO>) -> impl Responder {
    if let (Some(account), Some(state)) = (arg.0.account.as_ref(), arg.0.state.as_ref()) {
        if account == "00000000000" && *state == 0 {
            return HttpResponse::Ok().json(RespVO::<u64>::from_result(Err(Error::from(error_info!(
