...
pub async fn page(page: web::types::Json<ResPageDTO>) -> impl Responder {
    let data = CONTEXT.sys_permission_service.page(&page.0).await;
    RespVO::from_result(&data).resp_json()
}

pub async fn all(_page: web::types::Json<EmptyDTO>) -> impl Responder {
    let data = CONTEXT.sys_permission_service.finds_all().await;
    RespVO::from_result(&data).resp_json()
}

pub async fn layer_top(_page: web::types::Json<EmptyDTO>) -> impl Responder {
    let data = CONTEXT.sys_permission_service.finds_layer_top().await;
    RespVO::from_result(&data).resp_json()
}

pub async fn add(mut arg: web::types::Json<PermissionAddDTO>) -> impl Responder {
    if arg.name.is_none() {
...
pub async fn update(arg: web::types::Json<ResEditDTO>) -> impl Responder {
